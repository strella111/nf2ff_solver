...
from far_zone.near_field_data import (PHASE_LEVELS, amp_display, amp_levels,
                                      axis_layout, axis_points, field_image,
                                      field_stats, prepare_maps)
...
class LevelsTests(unittest.TestCase):
    def test_normalisation_puts_maximum_at_zero(self):
        img = np.array([[-10.0, -3.0], [-20.0, np.nan]])
        out = amp_display(img, normalize=True)
        self.assertAlmostEqual(np.nanmax(out), 0.0)
        self.assertAlmostEqual(out[0, 0], -7.0)
        np.testing.assert_array_equal(amp_display(img, normalize=False), img)

    def test_floor_keeps_contrast(self):
        """Одна провальная точка не должна съедать всю шкалу."""
        img = np.array([[0.0, -5.0], [-200.0, -3.0]])
        lo, hi = amp_levels(img, floor_db=-40.0)
        self.assertAlmostEqual(hi, 0.0)
        self.assertAlmostEqual(lo, -40.0)

    def test_levels_follow_data_when_above_floor(self):
        img = np.array([[0.0, -5.0], [-8.0, -3.0]])
        lo, hi = amp_levels(img, floor_db=-40.0)
        self.assertAlmostEqual(lo, -8.0)
        self.assertAlmostEqual(hi, 0.0)

    def test_flat_and_empty_fields_do_not_break(self):
        lo, hi = amp_levels(np.full((2, 2), -7.0))
        self.assertLess(lo, hi, 'у ровного поля шкала не должна схлопнуться')
        lo, hi = amp_levels(np.full((2, 2), np.nan))
